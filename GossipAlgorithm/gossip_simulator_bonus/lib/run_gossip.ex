defmodule RunGossip do
  ...
  def trigger_gossip(gossip_actors, actor_to_neighbours) do
    ...
    IO.puts "Start gossip Bruce Wayne is Batman"
  ...
  def termiante_process(actor_pid, gossip_actors, actor_to_neighbours, black_list) do
    ...
    info = Enum.reduce(gossip_actors, {}, fn(actor_pid, info) ->
        ...
        {gossip_actors, actor_to_neighbours, black_list}  = if gossip_neighbour_pid != 0 do
          ...
          {gossip_actors, actor_to_neighbours, black_list} = if(terminate) do
            termiante_process(gossip_neighbour_pid, gossip_actors, actor_to_neighbours, black_list)
          else
           {gossip_actors, actor_to_neighbours, black_list}
          end 
        else
          {gossip_actors, actor_to_neighbours, black_list} = termiante_process(actor_pid, gossip_actors, actor_to_neighbours, black_list)
